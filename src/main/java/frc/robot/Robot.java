...
public class Robot extends TimedRobot {
    ...
    public void autonomousPeriodic() {
        // putting the values onto smart dashboard
    ...
    public void localizationInit() {
        ...
        currentRotation = new Rotation2d(-Math.PI / 2); //This should be directly facing the opponents driver stations, just a starting value
        currentLocation = new Pose2d(154.875/*Inches*/, 0, currentRotation); // Distance in inches of the initiation line, which we will start on, but that y value is unknown and is set to 0
        limelight = NetworkTableInstance.getDefault().getTable("limelight");


        //random starter values
        ty = 0;
        a2 = ty;
        visionDistance = 0;
        rAngle = 0;
    ...
    public Pose2d localizationPeriodic() {
        ...
        if (limelight.getEntry("tv").getDouble(0) == 1) { //If it sees a target

            //logic to assign the important values
            ...
            if (turretHeading < Math.PI) { //Whether is it facing the opponents target or the allies target
